...
namespace CarFactoryView
{
    ...
    {
        private static TcpClient mailConsumer;
        ...
        public static void Connect()
        {
            ...
            mailConsumer = new TcpClient();
            mailConsumer.Connect("pop.gmail.com", 995);
            stream = new SslStream(mailConsumer.GetStream());
        ...
        /// <summary>
        /// Отправка запроса и получение ответа от почтового сервера
        /// </summary>
        ...
        private static void GetMessages(int number)
        {
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    {
                        APIConsumer.PostRequestData("api/MessageInfo/AddElement",
                            new BindingMessageInfo
                            {
                                MessageId = messageId,
        ...
        private static string GetMail(string from)
        {
            var begin = from.IndexOf("<") + 1;
            var end = from.IndexOf(">") - begin - 1;
            return from.Substring(begin, end + 1);
        }
    }
}